module VotesHelper
  def link_to_like(votable, vote, *args)
    ...
    link_text = text + ' ' + t('label.like') + ' ' + "(#{votable.votes.positives.count})"
    ...
    if vote.vote_flag
      ...
      link_path = polymorphic_path([votable, vote], vote_flag: 'positive')

      link_to link_text, link_path, options
    end
  end

  def link_to_dislike(votable, vote, *args)
    options = args.extract_options!
    ...
    link_text = text + ' ' + t('label.dislike') + ' ' + "(#{votable.votes.negatives.count})"
    ...
    if vote.vote_flag == false
      ...
      link_path = polymorphic_path([votable, vote], vote_flag: 'negative')
